...
<template>
  <header>
      ...
      <nav>
        ...
        <RouterLink to="/bans">
          <font-awesome-icon :icon="['fas', 'ban']" /> Bans
        </RouterLink>
        <RouterLink to="/appeal">
          <font-awesome-icon :icon="['fas', 'envelope']" /> Appeal
        </RouterLink>
        <RouterLink to="/servers">
          <font-awesome-icon :icon="['fas', 'server']" /> Servers
        </RouterLink>
      </nav>
      <div id="github">
        <a id="github-url" href=""><font-awesome-icon :icon="['fab', 'github']" /> <span id="github-stars">0</span></a>
      </div>
  </header>
  <div id="messagebar" style="display: none;" ref="messagebar">
...
const messagebar = ref<HTMLDivElement | null>(null)

onMounted(async () => {
  try {
    const response = await axios.get('/api/user')
    const data = response.data
    if (data.Banned === true && messagebar.value) {
      messagebar.value.style.display = 'flex';
    }
  } catch (error) {
